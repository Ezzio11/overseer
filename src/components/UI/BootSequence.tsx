...
export default function BootSequence({ onComplete }: { onComplete: () => void }) {
    ...
    useEffect(() => {
        // 1. Initial spin duration
        const spinTimer = setTimeout(() => {
            setIsExiting(true);
        }, 2500);

        // 2. Fade out complete -> trigger parent onComplete
        const completeTimer = setTimeout(() => {
            onComplete();
        }, 3100); // 2500 + 600ms buffer (transition is 500ms)
    ...
    return (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center pointer-events-none">
            {/* INLINE SVG STYLES REUSED BUT SCALED UP */}
            <style dangerouslySetInnerHTML={{
                __html: `
                    @keyframes boot-globe-spin {
                        from { transform: translateX(0px); }
                        to { transform: translateX(-189px); }
                    }
                    .boot-globe-container {
                        display: inline-block;
                        border-radius: 50%;
                        overflow: hidden;
                        width: 128px; /* Doubled size */
                        height: 128px; /* Doubled size */
                        border: 2px solid rgba(6, 182, 212, 0.5);
                        box-shadow: 0 0 40px rgba(6, 182, 212, 0.3);
                        position: relative;
                        transform: translate3d(0, 0, 0);
                        background: rgba(0,0,0,0.3); /* Slight tint to make globe pop against stars */
                        backdrop-filter: blur(2px);
                    }
                    .boot-globe-scroller {
                        width: 864px; /* Doubled from 432 */
                        height: 128px; /* Doubled from 64 */
                        display: block;
                        animation: boot-globe-spin 4s linear 0s infinite;
                        will-change: transform;
                    }
                `}} />

            <div
                className={`transition-all duration-500 ease-in-out transform ${isExiting ? 'opacity-0 scale-[5] blur-sm' : 'opacity-100 scale-100 blur-0'
                    }`}
            >
                <div className="boot-globe-container">
                    <svg className="boot-globe-scroller" viewBox="0 0 432 100">
                        <g transform="translate(0, 0)" fill="#06b6d4"><path d={WORLD_PATH} /></g>
                        <g transform="translate(189, 0)" fill="#06b6d4"><path d={WORLD_PATH} /></g>
                    </svg>
                </div>
            </div>
        </div>
